...
import { ArrowLeft, Upload, Download, BookOpen, Edit } from 'lucide-react';
import { GoogleGenAI, Modality } from '@google/genai';
import { imageUrlToBase64 } from '../utils/fileUtils';

const sampleImages = [
    { name: 'Animal', url: 'https://images.unsplash.com/photo-1474511320723-9a56873867b5?w=200&h=300&fit=crop' },
    { name: 'Retrato', url: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=200&h=300&fit=crop' },
];

export const ColoringBookScreen: React.FC<{ onBack: () => void; onEdit: (imageUrl: string) => void; initialImage?: string | null; }> = ({ onBack, onEdit, initialImage }) => {
    ...
    const handleProcess = async (imageUrl: string) => {
        ...
        try {
            ...
            const prompt = `Transform this image into a coloring book version by following these guidelines:
Use thin, well-defined black lines.
A simple, clean, and fluffy line style is ideal for coloring later.
Don't add shadows or colors: just outlines.
Maintain the main elements of the photo, simplifying complex details.
If there are people or objects, draw in a friendly, rounded style.
Leave well-defined spaces to make coloring easier.
Don't add new elements that aren't in the original photo.
Create the image in the traditional "coloring book" style: black lines on a white background.`;
            ...
            if (!imageFound) {
                 ...
                 if (textResponse) {
                    throw new Error(`A IA não gerou a imagem. Motivo: ${textResponse}`);
                 }
                throw new Error("A IA não retornou uma imagem. Tente novamente.");
            }
        } catch (err: any) {
            console.error("Erro na geração do Livro de Colorir:", err);
    ...
    const handleDownload = () => {
        ...
        link.download = `livro-de-colorir-${Date.now()}.jpg`;
        ...
        <div className="flex flex-col h-screen bg-[#1F1F1F] text-white overflow-hidden">
            ...
            <header className="flex-shrink-0 z-10 p-4">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-semibold text-lg drop-shadow-md">Livro de Colorir IA</h1>
                </div>
            </header>
            ...
            <main className="flex-1 min-h-0 flex flex-col items-center justify-center p-4">
                {isProcessing ? (
                    <div className="text-center">
                        ...
                        <p className="font-semibold">Criando suas linhas...</p>
                    </div>
                ) : processedImage ? (
                     <img src={processedImage} alt="Resultado" className="max-w-full max-h-full object-contain rounded-lg bg-white"/>
                ) : (
                    <div className="text-center max-w-2xl">
                       <BookOpen size={48} className="mx-auto text-blue-400 mb-4"/>
                        <h2 className="text-3xl font-bold mb-2">Livro de Colorir IA</h2>
                        <p className="text-gray-400 mb-6">Transforme qualquer foto em uma página de colorir pronta para impressão.</p>
