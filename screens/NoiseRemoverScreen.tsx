...
import { ArrowLeft, Upload, Download, ShieldCheck, Edit } from 'lucide-react';
import { GoogleGenAI, Modality } from '@google/genai';
import { imageUrlToBase64 } from '../utils/fileUtils';

const sampleImages = [
    { name: 'Concerto', url: 'https://images.unsplash.com/photo-1524368535928-5b5e00ddc76b?w=200&h=300&fit=crop' },
    { name: 'Retrato', url: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&h=300&fit=crop' },
    { name: 'Interior', url: 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=200&h=300&fit=crop' },
    { name: 'Noturno', url: 'https://images.unsplash.com/photo-1519125323398-675f0ddb6308?w=200&h=300&fit=crop' },
];
...
export const NoiseRemoverScreen: React.FC<{ onBack: () => void; onEdit: (imageUrl: string) => void; }> = ({ onBack, onEdit }) => {
    const [originalImage, setOriginalImage] = useState<string | null>(null);
    const [processedImage, setProcessedImage] = useState<string | null>(null);
    const [isProcessing, setIsProcessing] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [intensity, setIntensity] = useState<'Leve' | 'Padrão'>('Padrão');
    ...
    const handleRemoveNoise = async () => {
        ...
        try {
            ...
            const prompt = `Remova o ruído digital e a granulação desta imagem. A intensidade da remoção de ruído deve ser ${intensity === 'Leve' ? 'leve' : 'padrão'}. Melhore a nitidez e a clareza, mantendo a aparência natural da foto. Não altere o conteúdo, a composição ou as cores originais da imagem.`;
            ...
            for (const part of parts) {
             ...
             if (!newImageFound) {
                ...
                if (text) throw new Error(`Falha na IA: ${text}`);
                 if (candidate?.finishReason) {
                     throw new Error(`Processamento bloqueado: ${candidate.finishReason}`);
                 }
                throw new Error("A IA não retornou uma imagem. Tente novamente.");
            }
            
        } catch (err: any) {
            console.error("Erro ao remover ruído:", err);
    ...
    const handleDownload = () => {
        ...
        link.download = `noise-removed-${Date.now()}.png`;
    ...
    return (
        <div className="flex flex-col h-screen bg-[#1F1F1F] text-white overflow-hidden">
            ...
            <header className="flex-shrink-0 z-10 p-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="font-semibold text-lg drop-shadow-md">Removedor de Ruído IA</h1>
                    </div>
                    {processedImage && (
            ...
            <main className="flex-1 min-h-0 flex flex-col items-center justify-center p-4 relative">
                 {isProcessing && (
                    <div className="absolute inset-0 bg-zinc-900/80 z-10 flex flex-col items-center justify-center text-center p-4 backdrop-blur-sm">
                        ...
                        <h3 className="text-white font-semibold text-lg">Removendo ruído...</h3>
                        <p className="text-gray-300 text-sm mt-1">A IA está limpando sua imagem.</p>
                    </div>
                )}
                ...
                {originalImage && processedImage ? (
                    ...
                    <div className="w-full max-w-2xl text-center">
                        ...
                        <h2 className="text-3xl font-bold mb-2">Removedor de Ruído IA</h2>
                        <p className="text-gray-400 mb-6">Reduza o ruído e a granulação da imagem com um clique.</p>
            ...
            <footer className="flex-shrink-0 z-10 p-4">
                ...
                {originalImage && (
                    <div className="max-w-md mx-auto flex items-center gap-4 bg-black/30 backdrop-blur-sm p-4 rounded-xl">
                        <div className="flex-1 space-y-2">
                            <button onClick={() => setIntensity('Leve')} className={`w-full text-left p-2 bg-gray-700/50 rounded-lg border-2 flex justify-between items-center ${intensity === 'Leve' ? 'border-blue-500' : 'border-transparent hover:border-gray-500'}`}>
                               <span className="font-semibold text-sm">Leve</span>
                            </button>
                            <button onClick={() => setIntensity('Padrão')} className={`w-full text-left p-2 bg-gray-700/50 rounded-lg border-2 flex justify-between items-center ${intensity === 'Padrão' ? 'border-blue-500' : 'border-transparent hover:border-gray-500'}`}>
                               <span className="font-semibold text-sm">Padrão</span>
                            </button>
                        </div>
                        <button onClick={handleRemoveNoise} disabled={isProcessing} className="bg-blue-600 font-bold py-3 px-6 rounded-lg self-stretch flex items-center justify-center gap-2 disabled:bg-gray-500">
                            {isProcessing ? (
