...
export const CinematicPortraitScreen: React.FC<{ onBack: () => void; onEdit: (imageUrl: string) => void; }> = ({ onBack, onEdit }) => {
    ...
    const handleProcess = async (imageUrl: string) => {
        ...
        try {
            ...
            const prompt = "Generate a high-quality, hyper-realistic cinematic portrait of the person in the uploaded image. The style should mimic a high-end movie still or an editorial magazine cover. Camera: 85mm lens, f/1.8 aperture for a shallow depth of field. Lighting: Dramatic, volumetric lighting (Rembrandt style) with a soft rim light to separate the subject from the background. Details: Ultra-detailed skin texture, sharp focus on the eyes. Atmosphere: Moody, atmospheric, with a cinematic color grade (subtle teal and orange tones). Background: Blurred, neutral, or abstract cinematic background. Ensure the facial features and identity of the person are perfectly preserved.";
            ...
            console.error("Erro na geração do Retrato Cinematográfico:", err);
    ...
    const handleDownload = () => {
        ...
        link.download = `cinematic-portrait-${Date.now()}.jpg`;
        ...
        <div className="flex flex-col h-screen bg-[#1F1F1F] text-white overflow-hidden">
            ...
            <header className="flex-shrink-0 z-10 p-4">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-semibold text-lg drop-shadow-md">Retrato Cinematográfico</h1>
                </div>
            </header>
            ...
            <main className="flex-1 min-h-0 flex flex-col items-center justify-center p-4">
                {isProcessing ? (
                    <div className="text-center">
                        ...
                        <p className="font-semibold">Criando seu retrato cinematográfico...</p>
                    </div>
                ) : processedImage ? (
                    ...
                    <div className="text-center max-w-2xl">
                       <User size={48} className="mx-auto text-blue-400 mb-4"/>
                        <h2 className="text-3xl font-bold mb-2">Gerador de Retrato Cinematográfico IA</h2>
                        <p className="text-gray-400 mb-6">Transforme sua foto em um retrato elegante e artístico com um toque cinematográfico.</p>
